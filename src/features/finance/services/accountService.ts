...
// Convert row to Account object with calculated balance
...
const computeAccountBalances = (accounts: AccountRow[], transactions: any[]): Account[] => {
    return accounts.map((accountRow) => {
        ...
        return rowToAccount(accountRow, dynamicCurrentBalance, latestDate);
    });
};

// Account Service with Offline & Cache Support
